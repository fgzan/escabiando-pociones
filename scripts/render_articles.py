...
def render_card(kind, item):
    ...
    if kind == "review":
        pct = fmt_pct(item.get("score"))
        score_txt = f'{item["score"]} / 10' if item.get("score") is not None else ""
        return f'''<a class="card" href="{folder}/{slug}.html" style="display:block; margin-bottom:20px;">
      {cover}
      <span class="eyebrow">{esc(item.get("game", ""))}</span>
...
if __name__ == "__main__":
    ...
    for r in published_reviews:
        render_article_page("review", r)
    for n in published_noticias:
        render_article_page("noticia", n)
    ...
    build_sitemap(published_reviews, published_noticias)

    print(f"Listo: {len(published_reviews)} review(s), {len(published_noticias)} noticia(s) generadas como páginas estáticas.")
